/**
 * API易 BananaPro (Gemini 3 Pro Image) API Service
 * 支持文生图和图生图功能
 */
 ...
 */
export async function textToImage({ apiKey, prompt, aspectRatio = '1:1', resolution = '2K', signal }) {
  const payload = {
    contents: [{
      parts: [
        { text: prompt }
  ...
  return await callGeminiApi({ apiKey, payload, signal })
}

/**
 * 图生图 - Image to Image (编辑图片)
 * @param {Object} options
 * @param {string} options.apiKey - API Key
...
export async function imageToImage({
  ...
  if (Array.isArray(imageBase64)) {
    // 多张图片
    imageBase64.forEach((base64, index) => {
 ...
 * @returns {Promise<Object>}
 */
async function callGeminiApi({ apiKey, payload, signal }) {
  const headers = {
  ...
  try {
    ...
    for (const part of candidate.content.parts) {
      // 支持两种命名格式: inlineData 和 inline_data
